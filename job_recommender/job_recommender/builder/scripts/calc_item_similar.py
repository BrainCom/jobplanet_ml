...
from ...scores.models import Similarity
...
class ItemSimilarityMatrixBuilder(object):
    ...
    def build(self, ratings, save=True):
        ...
        ratings['avg'] = ratings.groupby('customer_id')['rating'].transform(lambda x: normalize(x))

        ratings['avg'] = ratings['avg'].astype(float)
        ratings['customer_id'] = ratings['customer_id'].astype('category')
        ratings['company_id'] = ratings['company_id'].astype('category')

        coo = coo_matrix((ratings['avg'].astype(float),
                          (ratings['company_id'].cat.codes.copy(),
                           ratings['customer_id'].cat.codes.copy())))
        ...
        companies = dict(enumerate(ratings['company_id'].cat.categories))
        ...
        if save:
            ...
            if self.db == 'django.db.backends.postgresql':
                self._save_similarities(cor, companies)
            else:
                self._save_with_django(cor, companies)

            logger.debug('save finished, done in {} seconds'.format(datetime.now() - start_time))

        return cor, companies
...
def load_all_ratings(min_ratings=1):
    columns = ['customer_id', 'company_id', 'rating', 'type']

    ratings_data = Rating.objects.all().values(*columns)

    ratings = pd.DataFrame.from_records(ratings_data, columns=columns)
    user_count = ratings[['customer_id', 'company_id']].groupby('customer_id').count()
    user_count = user_count.reset_index()
    user_ids = user_count[user_count['company_id'] > min_ratings]['customer_id']
    ratings = ratings[ratings['customer_id'].isin(user_ids)]
    ratings['rating'] = ratings['rating'].astype(float)
    return ratings
